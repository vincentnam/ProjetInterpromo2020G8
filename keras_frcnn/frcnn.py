...
from tensorflow.keras.layers import Layer, Input, Add, Dense, Activation, \
...
def transition_block(x, reduction, name):
    ...
    x = layers.Conv2D(int(K.int_shape(x)[bn_axis] * reduction), 1,
                      use_bias=False,
                      name=name + '_conv')(x)
    x = layers.AveragePooling2D(2, strides=2, name=name + '_pool',
                                padding='same')(x)
    return x
...
def classifier(base_layers, input_rois, num_rois, nb_classes=21,
    ...
    if K.backend() == 'tensorflow':
        pooling_regions = 14
        input_shape = (
        num_rois, 14, 14, 1024)  # densenet output channels are 1024..
    ...
    out_roi_pool = TimeDistributed(AveragePooling2D((7, 7)), name='avg_pool')(
        out_roi_pool)
